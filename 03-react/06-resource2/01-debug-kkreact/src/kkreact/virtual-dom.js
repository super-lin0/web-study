...
function updateVnode(vnode, newVnode, node, parentContext) {
  ...
  //更新原生
  let oldHtml = vnode.props[HTML_KEY] && vnode.props[HTML_KEY].__html; //HTML_KEY: dangerouslySetInnerHTML（危险使用，不建议）
  if (oldHtml != null) {
    updateVelem(vnode, newVnode, node, parentContext);
    initVchildren(newVnode, node, parentContext);
  } else {
    updateVChildren(vnode, newVnode, node, parentContext); //更新children
    updateVelem(vnode, newVnode, node, parentContext); //更新当前元素（属性、ref等）
  }
  return node;
}
...
function diffVchildren(patches, vnode, newVnode, node, parentContext) {
  ...
  if (vchildrenLen === 0) {
    if (newVchildrenLen > 0) {
      //新的children为空，并且新的vchildren不是空，证明有了新增子节点，则直接创建
    ...
    // 新的vchildren为空，证明老的children节点需要被删除了
...
export function compareTwoVnodes(vnode, newVnode, node, parentContext) {
  ...
  if (newVnode == null) {
    // remove
    destroyVnode(vnode, node); //删除掉虚拟dom树上的旧节点
    node.parentNode.removeChild(node); //删除dom上的节点
  } else if (vnode.type !== newVnode.type || vnode.key !== newVnode.key) {
    // replace
    destroyVnode(vnode, node); //删除掉虚拟dom树上的旧节点
    newNode = initVnode(newVnode, parentContext); //生成最新的dom节点
